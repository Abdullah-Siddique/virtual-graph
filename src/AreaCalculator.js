...
function AreaCalculator({ onGoBack }) {
  ...
  const handleAddPoint = () => {
    const newPoints = inputs.map((input) => [Number(input.x), Number(input.y)]);
    setPoints(newPoints);
    let calculatedArea = calculateArea(newPoints);
    
    // If the area is zero, reorder the points and recalculate the area
    if (calculatedArea === 0) {
      const reorderedPoints = reorderPoints(newPoints);
      setPoints(reorderedPoints);
      calculatedArea = calculateArea(reorderedPoints);
    }
  ...
  // Function to reorder points in a counter-clockwise direction
  const reorderPoints = (points) => {
    ...
    // Sort points based on their angle from the centroid (counter-clockwise order)
